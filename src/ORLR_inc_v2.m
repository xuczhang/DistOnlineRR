function [ beta, new_beta_arr, new_mc ] = ORLR_inc_v2(beta_arr, mc, Xtr, y)
...
diff_set_idx = randi(size(diff_set,2));
rm_idx = diff_set(diff_set_idx);
% replace the first corrupted block with new block
new_beta_arr = [beta_arr beta_new];
