function [ beta, AS ] = OnlineRLHH_v2(X, y)
%   OnlineRLHH_v2: use the floor(m/2) task who is closest to averaged beta
%   as correct set. Then remove the uncorrupted samples of the correct from
%   total sample set.
p = size(X, 1);
n = size(X, 2);
beta = zeros(p, 1);
